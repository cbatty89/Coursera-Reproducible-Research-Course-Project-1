...
```{r echo = FALSE}
setwd("E:/Learning/Coursera/Data Science Specialization/Reproducible Research/Week 2/Project")
```
...
The mean total steps per day is `r step_mean` and the median total steps is `r step_median`.
...
The new average is `r step_mean` and the new median is `r step_median_c`.  As we can see imputing these values has pushed the median to match the average.
